...
exports.authCheckProfile = async (req,res,next) => {
  ...
  try {
    ...
    console.log('Frontend to backend data', req.body)
    const {name, email, phone_no } = req.body.profileData;
    const profile = await User.findOneAndUpdate({email: email}, {name,phone: phone_no,}, {new:true});
    req.profile = profile;
...
exports.authCheckGetProfile = async (req,res,next) => {
  try {
    console.log(req.params.token)
    const firebaseUser = await admin.auth().verifyIdToken(req.params.token);
    console.log('<<<<<<<<FIREBASE USER IN AUTHCHECK Middleware>>>>>>>>', firebaseUser);
...
exports.adminCheckCreateP = async (req, res, next) => {
  try {
        ...
        const currentUser = await User.findOne({email}).exec();
          ...
          req.body.product.user_id = currentUser._id;
...
exports.adminCheckUpdateP =  async (req, res, next) => {
  try {
        ...
        req.user = firebaseUser;
        const adminUser = await User.findOne({email}).exec();
        const productUserID = await Product.findOne( {user_id: adminUser._id} , {user_id: 1}).exec()
        if(adminUser.role !== 'admin'){
          // console.log("###############" ,`${adminUser._id}` === `${productUserID.user_id}`)
          // const condition = `${adminUser._id}` === `${productUserID.user_id}`
          // console.log("**********" , condition)
          if (productUserID && `${adminUser._id}` === `${productUserID.user_id}`){
...
exports.productUploadedByUser =  async (req, res, next) => {
  try {    
        console.log("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
        const firebaseUser = await admin.auth().verifyIdToken(req.params.authtoken);
        console.log('<<<<<<<<FIREBASE USER IN AUTHCHECK Middleware ADMIN>>>>>>>>', firebaseUser);
        const {email} = firebaseUser;
        const adminUser = await User.findOne({email}).exec();
        const products = await Product.find( {user_id: adminUser._id}).exec()
